...
import { Settings, BarChart3, ListOrdered, Calendar, User, Save } from 'lucide-react';
...
function PolicyTab() {
  ...
  const [form, setForm] = useState({ sick: '', casual: '', earned: '' });
...
function BalancesTab() {
  ...
  const getEdit = (id, key, fallback) => editMap[id]?.[key] ?? fallback;

  const setEdit = (id, key, val) =>
    setEditMap((prev) => ({ ...prev, [id]: { ...(prev[id] || {}), [key]: val } }));

  const handleSave = async (user, balance) => {
