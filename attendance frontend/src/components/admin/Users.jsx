          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                className="bg-slate-900 border border-slate-700 rounded-lg px-3 py-1.5 text-sm text-white focus:outline-none focus:border-violet-500 transition-colors"
              >
        ...
        {isUsersLoading ? (
          ...
          <div className="bg-red-500/10 border border-red-500/20 text-red-400 p-4 rounded-xl flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">{usersError?.data?.message || usersError?.error || 'Unknown error'}</p>
            </div>
          </div>
        ) : users.length === 0 ? (
          <div className="py-14 text-center">
            <Users className="w-12 h-12 text-slate-700 mx-auto mb-3" />
            <p className="text-slate-500 text-sm">No users found matching the selected filter.</p>
            <button
              onClick={() => setShowAddModal(true)}
              className="mt-4 inline-flex items-center gap-2 text-violet-400 hover:text-violet-300 text-sm font-medium transition-colors cursor-pointer"
            >
              <UserPlus className="w-4 h-4" />
              Add your first user
            </button>
                ...
                <tbody className="divide-y divide-slate-800/40 text-sm">
                  {users.map((u) => (
                    <tr key={u._id} className="hover:bg-slate-900/30 transition-colors">
                      <td className="py-4 px-5 font-medium text-white">{u.name}</td>
                      ...
                      <td className="py-4 px-5 text-slate-400">{formatDate(u.createdAt)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
