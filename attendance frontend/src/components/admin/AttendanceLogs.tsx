...
import { useGetAllAttendanceQuery, useValidateAttendanceMutation } from '../../redux/api/attendanceApi';
import { Calendar, RefreshCw, AlertCircle, UserCheck, X, ExternalLink, Download } from 'lucide-react';
...
export default function AttendanceLogs() {
  ...
  const attendanceLogs: AttendanceLogItem[] = attendanceResponse?.data?.attendances || [];
  ...
  const handleValidate = async (attendanceId: string, status: 'valid' | 'invalid', remarksVal = '') => {
    ...
    setRemarks('');
    try {
      await validateAttendance({ attendanceId, status, remarks: remarksVal }).unwrap();
    } catch (err: any) {
  ...
  const handleExportCsv = () => {
    ...
    exportToCsv(`all-attendance-logs-${new Date().toISOString().split('T')[0]}.csv`, attendanceLogs, columns);
  };
      ...
      {isAttendanceLoading ? (
        ...
        <div className="bg-red-500/10 border border-red-500/20 text-red-600 dark:text-red-400 p-4 rounded-xl flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-theme-muted">{getErrorMsg(attendanceError)}</p>
          </div>
        </div>
