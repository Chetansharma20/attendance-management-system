...
export default function TeamLeaves() {
  ...
  const [actionMap, setActionMap] = useState<Record<string, ActionItem>>({}); // { leaveId: { mode: 'approve'|'reject', note: '' } }
  ...
  const leaves: LeaveItem[] = data?.data || [];

  const openAction = (leaveId: string, mode: 'approve' | 'reject') => {
    setActionMap((prev) => ({ ...prev, [leaveId]: { mode, note: '' } }));
  };

  const cancelAction = (leaveId: string) => {
    setActionMap((prev) => { const n = { ...prev }; delete n[leaveId]; return n; });
  };

  const handleAction = async (leaveId: string) => {
