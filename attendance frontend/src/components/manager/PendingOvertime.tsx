...
import { useGetPendingOvertimeQuery, useUpdateOvertimeStatusMutation } from '../../redux/api/overtimeApi';
...
export default function PendingOvertime() {
  ...
  const pendingOvertime: OvertimeRequest[] = pendingOvertimeResponse?.data || [];

  const handleOvertimeStatus = async (requestId: string, status: string) => {
    let rejectionReason = '';
    if (status === 'rejected') {
      ...
      {isPendingOvertimeLoading ? (
        ...
        <div className="bg-red-500/10 border border-red-500/20 text-red-600 dark:text-red-400 p-4 rounded-xl flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-theme-muted">{getErrorMsg(pendingOvertimeError)}</p>
          </div>
        </div>
              ...
              return (
                <div key={req._id} className="bg-theme-bg/20 border border-theme-border rounded-xl p-4 space-y-3 transition-colors duration-200">
                  <div className="flex justify-between items-start gap-2">
                    ...
                    <span className="text-xs font-bold font-mono text-theme-bright bg-theme-card px-2.5 py-1 rounded-lg border border-theme-border whitespace-nowrap">
                      {req.requestedHours} hr{req.requestedHours !== 1 ? 's' : ''}
                    </span>
                  </div>
                  ...
                  <div className="flex gap-2 pt-2 border-t border-theme-border/60">
                    <button
                      onClick={() => handleOvertimeStatus(req._id, 'approved')}
                      disabled={isUpdatingOvertime}
                      className="flex-1 inline-flex items-center justify-center gap-1.5 bg-emerald-600 hover:bg-emerald-500 disabled:bg-emerald-600/40 text-white font-semibold text-xs py-2 rounded-lg transition-colors cursor-pointer shadow-sm"
                    >
                      <ThumbsUp className="w-3.5 h-3.5" />
                      <span>Approve</span>
                    </button>
                    <button
                      onClick={() => handleOvertimeStatus(req._id, 'rejected')}
                      disabled={isUpdatingOvertime}
                      className="flex-1 inline-flex items-center justify-center gap-1.5 bg-red-600 hover:bg-red-500 disabled:bg-red-600/40 text-white font-semibold text-xs py-2 rounded-lg transition-colors cursor-pointer shadow-sm"
                    >
                      <ThumbsDown className="w-3.5 h-3.5" />
                      <span>Reject</span>
                    </button>
