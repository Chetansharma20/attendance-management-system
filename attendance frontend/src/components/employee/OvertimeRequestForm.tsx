...
import { useRequestOvertimeMutation } from '../../redux/api/overtimeApi';
...
export default function OvertimeRequestForm({ attendanceId, onSuccess, onCancel }: OvertimeRequestFormProps) {
  const [requestedHours, setRequestedHours] = useState<string>('');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError('');
    if (!requestedHours || isNaN(Number(requestedHours)) || Number(requestedHours) <= 0) {
