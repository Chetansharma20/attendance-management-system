...
export interface OvertimeRequestInfo {
  _id?: string;
  status: string;
  duration?: number;
  reason?: string;
...
export interface AttendanceLogItem {
  ...
  overtimeRequest?: OvertimeRequestInfo;
}
...
export default function DailyReportModal({
  ...
  const getFirstPunchInObj = (punches?: Punch[]): Punch | null => {
    if (!punches || punches.length === 0) return null;
    return punches.find(p => p.type === 'in') || null;
  };

  const getLastPunchOutObj = (punches?: Punch[]): Punch | null => {
    if (!punches || punches.length === 0) return null;
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-theme-bg/10" id="printable-report-area">
          ...
          <div className="mb-6 space-y-1">
            ...
            {isSingleEmployee && employeeName && (
              <p className="text-xs text-theme-muted print:text-slate-650">Employee: <span className="font-semibold text-theme-text">{employeeName}</span></p>
            )}
                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-theme-border/60 text-sm">
                    {filteredLogs.map((log) => {
                      ...
                      const firstIn = getFirstPunchInObj(log.punches);
                      const lastOut = getLastPunchOutObj(log.punches);
                      ...
                      return (
                        <tr key={log._id} className="hover:bg-theme-card-hover/30 transition-colors">
                          ...
                          <td className="py-4 px-4 align-middle font-mono text-xs text-theme-text print:text-black">
                            {firstIn?.time ? new Date(firstIn.time).toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit' }) : '-'}
                          </td>
                          ...
                          <td className="py-4 px-4 align-middle text-xs font-mono text-theme-text print:text-black">
                            {firstIn?.location?.latitude ? (
                              <div className="flex items-center gap-1.5">
                                <MapPin className="w-3.5 h-3.5 text-sky-600 dark:text-sky-400 shrink-0 print:hidden" />
                                <span>{firstIn.location.latitude.toFixed(4)}, {firstIn.location.longitude.toFixed(4)}</span>
                                <a
                                  href={`https://www.google.com/maps?q=${firstIn.location.latitude},${firstIn.location.longitude}`}
                                  target="_blank"
                          ...
                          {/* Punch Out Time */}
                          <td className="py-4 px-4 align-middle font-mono text-xs text-theme-text print:text-black">
                            {lastOut?.time ? new Date(lastOut.time).toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit' }) : '-'}
                          </td>
                          ...
                          {/* Punch Out Location */}
                          <td className="py-4 px-4 align-middle text-xs font-mono text-theme-text print:text-black">
                            {lastOut?.location?.latitude ? (
                              <div className="flex items-center gap-1.5">
                                <MapPin className="w-3.5 h-3.5 text-sky-600 dark:text-sky-400 shrink-0 print:hidden" />
                                <span>{lastOut.location.latitude.toFixed(4)}, {lastOut.location.longitude.toFixed(4)}</span>
                                <a
                                  href={`https://www.google.com/maps?q=${lastOut.location.latitude},${lastOut.location.longitude}`}
                                  target="_blank"
                                  rel="noopener noreferrer"
                                  className="text-violet-650 dark:text-violet-400 hover:text-violet-500 inline-flex items-center gap-0.5 ml-1 transition-colors print:hidden"
                                >
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2 border-t border-theme-border/60">
                        ...
                        <div className="space-y-1.5 text-xs">
                          ...
                          <p className="font-mono text-theme-text">
                            Time: {firstIn?.time ? new Date(firstIn.time).toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit' }) : '-'}
                          </p>
                          {firstIn?.selfieUrl ? (
                            <img src={firstIn.selfieUrl} alt="In Selfie" className="w-24 aspect-[4/3] object-cover rounded border border-theme-border" />
                          ) : (
                          ...
                          {firstIn?.location?.latitude ? (
                            <div className="flex items-center gap-1 font-mono text-theme-muted text-[10px]">
                              <span>{firstIn.location.latitude.toFixed(4)}, {firstIn.location.longitude.toFixed(4)}</span>
                              <a href={`https://www.google.com/maps?q=${firstIn.location.latitude},${firstIn.location.longitude}`} target="_blank" rel="noopener noreferrer" className="text-violet-650 dark:text-violet-400 hover:text-violet-500"><ExternalLink className="w-3 h-3" /></a>
                            </div>
                          ) : (
                        ...
                        <div className="space-y-1.5 text-xs border-t sm:border-t-0 sm:border-l border-theme-border/60 pt-3 sm:pt-0 sm:pl-3">
                          ...
                          <p className="font-mono text-theme-text">
                            Time: {lastOut?.time ? new Date(lastOut.time).toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit' }) : '-'}
                          </p>
                          {lastOut?.selfieUrl ? (
                            <img src={lastOut.selfieUrl} alt="Out Selfie" className="w-24 aspect-[4/3] object-cover rounded border border-theme-border" />
                          ) : (
                            <p className="text-xs text-theme-muted italic">No selfie</p>
                          )}
                          {lastOut?.location?.latitude ? (
                            <div className="flex items-center gap-1 font-mono text-theme-muted text-[10px]">
                              <span>{lastOut.location.latitude.toFixed(4)}, {lastOut.location.longitude.toFixed(4)}</span>
                              <a href={`https://www.google.com/maps?q=${lastOut.location.latitude},${lastOut.location.longitude}`} target="_blank" rel="noopener noreferrer" className="text-violet-650 dark:text-violet-400 hover:text-violet-500"><ExternalLink className="w-3 h-3" /></a>
                            </div>
                          ) : (
                            <p className="text-xs text-theme-muted italic">No location</p>
                          )}
