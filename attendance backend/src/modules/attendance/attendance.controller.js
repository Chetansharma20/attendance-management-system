...
export const punchIn = asyncHandler(async (req, res) => {
  ...
  const attendance = await punchInService(req.user._id, latitude, longitude, selfieUrl);
  res.status(201).json(new ApiResponse(201, attendance, "Punch in successful"));
});

export const punchOut = asyncHandler(async (req, res) => {
  const { latitude, longitude, selfieUrl } = req.body;
  const attendance = await punchOutService(req.user._id, latitude, longitude, selfieUrl);
  res.status(200).json(new ApiResponse(200, attendance, "Punch out successful"));
});

export const getMyAttendance = asyncHandler(async (req, res) => {
  const attendances = await getMyAttendanceService(req.user._id, req.query.date);
...
export const getAttendanceByEmployeeId = asyncHandler(async (req, res) => {
  const attendance = await getAttendanceByEmployeeIdService(req.params.employeeId);
...
export const getTeamAttendance = asyncHandler(async (req, res) => {
  const attendances = await getTeamAttendanceService(req.user._id);
  res.status(200).json(new ApiResponse(200, attendances, "Attendance fetched successfully"));
});
